...
from src.alerts.approved_cases_no_po_alert import ApprovedCasesNoPOAlert
...
def register_alerts(scheduler: AlertScheduler, config: AlertConfig) -> None:
    ...
    alert = ApprovedCasesNoPOAlert(config)
    scheduler.register_alert(alert.run)
    logger.info("[OK] Registered Approved Cases No PO Alert")
